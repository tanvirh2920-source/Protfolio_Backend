...
const getSettings = catchAsyncError(async (req, res) => {
  let result = await pool.query('SELECT * FROM portfolio_settings WHERE id = 1');
  if (result.rows.length === 0) {
    result = await pool.query('INSERT INTO portfolio_settings DEFAULT VALUES RETURNING *');
  }
...
const updateSettings = catchAsyncError(async (req, res) => {
  const {
    ...
    stat_projects, stat_technologies, stat_repos, stat_coffee,
    about_text,
  } = req.body;

  const result = await pool.query(
    `INSERT INTO portfolio_settings
       (id, full_name, title, bio, email, location, github_url, linkedin_url, twitter_url, resume_url,
     ...
     ON CONFLICT (id) DO UPDATE SET
       ...
       twitter_url=EXCLUDED.twitter_url, resume_url=EXCLUDED.resume_url,
